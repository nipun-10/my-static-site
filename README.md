# ☁️ Nipun Bhardwaj | Cloud Engineer & Solutions Architect Portfolio
Website link-https://nipun-10.github.io/portfolio-website/
### Professional Experience
* **Cloud Managed Services Intern** at **Velocis** (November 2025 - Present, Noida, India).
    * Focus on AWS operations, monitoring (CloudWatch alerts), and L1 support.
    * Working with core AWS services and documenting SOPs.

### Education
* **B.Tech** in **Computer Science and Engineering (Information Security)** from **Vellore Institute of Technology** (July 2021 - June 2025).

---

## 🏆 Certifications

The following certifications and training badges are highlighted in the portfolio:

* **AWS Certified Cloud Practitioner**
* **AWS SimuLearn Completion**
* **Introduction to Amazon CloudFront** (AWS)
* **AWS Compute Services Overview** (AWS)
* **Getting Started with Networking** (AWS Educate - Training Badge)
* **Cloud Computing Essentials** (AWS SimuLearn)
* **MERN Full Stack Internship** (Full Stack Development)

---

## 🖼️ Project Screenshots

To fully represent the project, the following screenshots of the deployed website should be included. **(Ensure these files are uploaded to the root directory of your repository)**.

### Screenshot 1: Hero & Primary Layout
![Hero Section Layout](Portfolio_img1.png)

### Screenshot 2: About Me & Contact Info
![About and Contact Section](Portfolio_img2.png)

### Screenshot 3: Core Skills Grid
![Detailed Skills Breakdown](Portfolio_img3.png)

### Screenshot 4: Professional Experience
![Experience Section (e.g., Velocis Internship)](Portfolio_img4.png)

### Screenshot 5: Certifications Badges
![Certifications & Badges Display](Portfolio_img5.png)

---

## 🤝 Get In Touch

I am open to discussing cloud architecture, DevOps practices, and potential opportunities.

* **Email:** nipuntyagi983@gmail.com
* **LinkedIn:** [https://www.linkedin.com/in/nipun-bhardwaj-6312a9265](https://www.linkedin.com/in/nipun-bhardwaj-6312a9265)
* **GitHub:** [https://github.com/nipun-10](https://github.com/nipun-10)
